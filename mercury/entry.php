...
<li id="prologue-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	// Write appropriate text above status posts.
	$post_format = p2_get_post_format( $post->ID );
	if ($post_format == 'status') {
		switch ( $timeframe = get_post_meta( $post->ID, 'timeframe', true ) ) {
			case 'thisweek':
				echo "<span class=\"timeframePostText\">This Week I'm Going to: </span>";
				break;
			case 'today':
				echo "<span class=\"timeframePostText\">Today I'm Going to: </span>";
				break;
			case 'tomorrow':
				echo "<span class=\"timeframePostText\">Tomorrow I'm Going to: </span>";
				break;
			case 'yesterday':
				echo "<span class=\"timeframePostText\">Yesterday I Accomplished: </span>";
				break;
			case 'lastweek':
				echo "<span class=\"timeframePostText\">Last Week I Accomplished: </span>";
				break;
		}
	}
